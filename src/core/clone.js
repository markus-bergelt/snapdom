...
function intersectsClip(b, rect) {
	return b.right >= rect.left - CLIP_CULL_MARGIN && b.left <= rect.right + CLIP_CULL_MARGIN && b.bottom >= rect.top - CLIP_CULL_MARGIN && b.top <= rect.bottom + CLIP_CULL_MARGIN;
}
	...
	try {
		...
		if (clone.attributes?.length) {
			try {
				...
				/* read-only attr or live collection change — non-blocking */
			}
		}
